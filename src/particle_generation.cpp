														...
														int n_particles) 
{
	...
	// Computes probability with which to select each particle class in particle_distributions
	...
	for (size_t ix = 0; ix < n_particles; ix++) {
		// Randomly selects which particle distribution to use for this particle
		ParticleDistribution pd = particle_distributions.at(d(mt));
		std::function<double(void)> sampler = get_sampler(pd.volume_distribution, mt);
